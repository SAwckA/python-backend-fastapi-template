...
class JWTConfigClass(BaseModel):
    """Настройка JWT"""
    jwt_alg: str = "HS512" \
        if env_get('JWT_ALG') is None \
        else env_get('JWT_ALG')

    access_token_exp: timedelta = timedelta(days=30) \
...
class DatabaseConfigClass(BaseModel):
    ...
    DB_HOST: str = "localhost" \
        if env_get("DB_HOST") is None \
        else env_get("DB_HOST")

    DB_PORT: int = 5432 \
        if env_get("DB_PORT") is None \
        else env_get("DB_PORT")
