...
async def get_user_by_username(username: str) -> None | schemas.UserLogin:
...
async def insert_new_user(username: str,
                          password: str,
                          email: str,
                          fio: str) -> bool:
    """
    ...
    async with db.pool.acquire() as conn:
        try:
            await conn.execute(sql, username, password, email, fio)
