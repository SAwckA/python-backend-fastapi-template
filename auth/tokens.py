...
payload_class_hint = AccessPayload \
                     | RefreshPayload \
                     | BaseTokenPayload

payload_class_exception_hint = AccessPayload \
                               | RefreshPayload \
                               | BaseTokenPayload \
                               | HTTPException
...
class AbstractJWT(ABC):
    ...
    @abstractmethod
    def check_sub(self,
                  token_payload: payload_class_hint) \
            -> payload_class_exception_hint:
...
class BaseJWT(AbstractJWT):
    ...
    def encode_token(cls,
        ...
        token = json_web_token.encode(payload=payload,
                                      key=SECRET_KEY,
    ...
    def validate_token(cls, token: str) -> payload_class_exception_hint:
        ...
        except (json_web_token.ExpiredSignatureError,
                json_web_token.DecodeError) as e:
...
class JWTRefreshToken(BaseJWT):
    ...
    def encode_token(cls,
                     data: payload_class_hint,
                     sid: str = "",
                     token_type: str = token_type) -> tuple[str, str]:
